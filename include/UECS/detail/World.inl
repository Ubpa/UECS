...
namespace Ubpa {
	...
	void World::Each(Sys&& s) {
		...
		detail::World_::Each<ArgList, TaggedCmptList, OtherArgList>::run(entityMngr, std::forward<Sys>(s));
	}
	...
	void World::ParallelEach(Sys&& s) {
		using ArgList = FuncTraits_ArgList<Sys>;
		using TaggedCmptList = CmptTag::GetTimePointList_t<ArgList>;
		detail::World_::ParallelEach<TaggedCmptList>::run(entityMngr, executor, std::forward<Sys>(s));
	}
...
//		using AllList = CmptTag::ConcatedAll_t<TypeList<Args...>>;
//		using AnyList = CmptTag::ConcatedAny_t<TypeList<Args...>>;
//		using NoneList = CmptTag::ConcatedNone_t<TypeList<Args...>>;
